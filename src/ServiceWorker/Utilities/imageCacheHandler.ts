...
const getWidth = (url: URL) =>
    Number(new URLSearchParams(url.search).get('width'));

const isImage = (url: URL) => imageRegex.test(url.pathname);
 ...
 * @returns {boolean}
 */
export const isResizedImage = ({ url }: { url: URL }) =>
 ...
 */
export const findSameOrLargerImage = async (url: URL) => {
    ...
    if (best.candidate) {
        ...
        // console.log(`ServiceWorker responding to GET ${url.pathname} at ${requestedWidth}w with cached version ${best.difference}px larger: ${bestCachedResponse?.url}`);

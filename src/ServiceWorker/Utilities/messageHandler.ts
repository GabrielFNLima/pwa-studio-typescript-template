 ...
 */
export const registerMessageHandler = (type: string, handler: Function) => {
    if (!handlers[type]) {
        handlers[type] = [];
    }
 ...
 * @returns {void}
 */
export const unRegisterMessageHandler = (type: string, handler: Function) => {
    if (handlers[type]) {
...
export const sendMessageToWindow = (type: string, payload: object) =>
    // @ts-ignore
    clients.matchAll().then((clients: any) => {
        const [windowClient] = clients.filter(
            (client: any) => client.type === 'window'
        );
